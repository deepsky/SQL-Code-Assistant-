...
public class TNSSymbolTableGenerator {
    ...
    final String SYMBOL_TABLE_PATH = "com/deepsky/generated/tns";
    final String TOKEN_TABLE_PATH = "com/deepsky/integration/lexer/generated/tns";

    File srcDir;
    int tableSize = 0;

    // OPEN_PAREN=23
    Pattern lexerPat = Pattern.compile("^([A-Z_]+)=([0-9]+)");
//    Pattern lexerPat = Pattern.compile("^(.*)=([0-9]+)");
    ...
    /**
     *
     * @param parser_tab TNSTokenTypes.txt
     * @param srcPath
     * @throws java.io.IOException
     */
    public TNSSymbolTableGenerator(File parser_tab, File srcPath) throws IOException {
        // 1. load Lexer table

        BufferedReader r1 = new BufferedReader(new FileReader(parser_tab));
        String line;
        int lineCounter = 0;
        while ((line = r1.readLine()) != null) {
            ...
            if (++lineCounter < 3) {
                ...
                if (m.find()) {
                    ...
                    int tokenCodeInt = Integer.parseInt(tokenCode);
                    lexerTokens.put(tokenName, tokenCodeInt);

                    tableSize = Math.max(tableSize, tokenCodeInt);
                }
            }
        }
        r1.close();
...
*/
        ...
        String templ = "\t\tIElementType <token_name> = new TNSTokenType(<token_id>, \"<keyword>\");";

        String static_end = "}\n";

        File path = new File(srcDir, TOKEN_TABLE_PATH);
        OutputStream out = new FileOutputStream(new File(path, "TNSPsiBaseTokenTypes.java"));
        ...
        for (int i = 0; i < tokenNames.size(); i++) {
            ...
            String t = "\t\t" + tokenName; //createTableItem(tokenName);
            out.write((t + ",\n").getBytes());
        }

        // put border array element
        out.write("\t\tnew TNSTokenType(9999, \"$$$END_OF_ARRAY_$$$\")\n".getBytes());
 ...
 */
        ...
        String static_end = "\t}\n}\n";
        String templ = "\t\ttable[<token_id>] = TNSPsiBaseTokenTypes.<token_name>;";

        File path = new File(srcDir, TOKEN_TABLE_PATH);
        ensureDirExists(path);
        OutputStream out = new FileOutputStream(new File(path, "TNSTokenTypesMapping.java"));
...
*/
    ...
    static public void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        System.out.println("TNS SymbolTableGenerator");
        if (args == null || args.length != 2) {
            System.out.println("Arguments less then expected. Expected 3, actual: " + args.length);
            System.exit(-1);
        }

        File parser_tab = new File(args[0]);
        System.out.println("1st argument: " + parser_tab.getAbsolutePath());
        if (!parser_tab.exists()) {
            System.out.println("Could not find TNS parser Table: " + parser_tab.getAbsolutePath());
            System.exit(-1);
        }
...
*/
        ...
        File sym_tab = new File(args[1]);
        System.out.println("3d argument: " + args[1]);
...
*/
        ...
        TNSSymbolTableGenerator gen = new TNSSymbolTableGenerator(parser_tab, sym_tab);
//        gen.setKeywordExtSources(keywordExtSources);

        System.out.println("Run TNS Symbol Table Generator");
        gen.generateClasses();
