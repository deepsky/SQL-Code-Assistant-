...
import com.deepsky.findUsages.SqlUsageGroupingRuleProviderImpl;
import com.deepsky.findUsages.providers.UsageFilteringRuleProvider2Impl;
 ...
 * @author max
 */
public class UsageView2Impl implements UsageView, UsageModelTracker.UsageModelTrackerListener {
    @NonNls
    ...
    private static UsageFilteringRuleProvider filteringRuleProvider = new UsageFilteringRuleProvider2Impl();

    public UsageView2Impl(@NotNull Project project,
...
// collisions with RubyMine            myCentralPanel.add(ScrollPaneFactory.createScrollPane(myTree), BorderLayout.CENTER);
            myCentralPanel.add(new JScrollPaneEe(myTree), BorderLayout.CENTER);
...
//    final UsageFilteringRuleProvider[] providers = Extensions.getExtensions(UsageFilteringRuleProvider.EP_NAME);
        java.util.List<UsageFilteringRule> list = new ArrayList<UsageFilteringRule>(); //(providers.length);
//    for (UsageFilteringRuleProvider provider : providers) {
//      list.addAll(Arrays.asList(provider.getActiveRules(project)));
//    }
        UsageFilteringRuleProvider provider = filteringRuleProvider; //new UsageFilteringRuleProvider2Impl();
        list.addAll(Arrays.asList(provider.getActiveRules(project)));
    ...
    private static UsageGroupingRule[] getActiveGroupingRules(final Project project) {
        ...
        java.util.List<UsageGroupingRule> list = new ArrayList<UsageGroupingRule>(); //providers.length);
        UsageGroupingRuleProvider provider = new SqlUsageGroupingRuleProviderImpl();
        list.addAll(Arrays.asList(provider.getActiveRules(project)));
    ...
    private AnAction[] createActions() {
        final TreeExpander treeExpander = new TreeExpander() {
            ...
            public void collapseAll() {
                UsageView2Impl.this.collapseAll();
            }
        ...
        myTextFileExporter = null; // tood -- !!! new ExporterToTextFile(this);
...
//    final UsageGroupingRuleProvider[] providers = Extensions.getExtensions(UsageGroupingRuleProvider.EP_NAME);
        java.util.List<AnAction> list = new ArrayList<AnAction>(); //providers.length);
//    for (UsageGroupingRuleProvider provider : providers) {
//        if(!(provider instanceof UsageGroupingRuleProviderImpl)){
...
//    }
    ...
    private void rulesChanged() {
        ...
        final java.util.List<Usage> allUsages = new ArrayList<Usage>(myUsageNodes.keySet());
    ...
    private class MergeDupLines extends RuleAction {
        private MergeDupLines() {
            super(UsageView2Impl.this, UsageViewBundle.message("action.merge.same.line"), IconLoader.getIcon("/toolbar/filterdups.png"));
    ...
    private void doReRun() {
        ProgressManager.getInstance().run(new Task.Backgroundable(myProject, UsageViewManagerImpl.getProgressTitle(myPresentation)) {
            public void run(@NotNull final ProgressIndicator indicator) {
                ...
                // todo [idea12] final com.intellij.usages.UsageViewManager usageViewManager = com.intellij.usages.UsageViewManager.getInstance(myProject);
                // todo [idea12] usageViewManager.setCurrentSearchCancelled(false);
                final com.intellij.usageView.UsageViewManager usageViewManager = com.intellij.usageView.UsageViewManager.getInstance(myProject);
                usageViewManager.closeContent(myContent);

                myChangesDetected = false;
                UsageSearcher usageSearcher = myUsageSearcherFactory.create();
                usageSearcher.generate(new Processor<Usage>() {
                    public boolean process(final Usage usage) {
                        // todo [idea12] if (usageViewManager.searchHasBeenCancelled()) return false;
                        usageViewManager.closeContent(myContent);
    ...
    public UsageNode doAppendUsage(Usage usage) {
        ...
        myUsages.add(usage);
// TODO [idea12]        UsageNode node = myBuilder.appendUsage(usage);
    ...
    public void selectUsages(@NotNull Usage[] usages) {
        java.util.List<TreePath> paths = new LinkedList<TreePath>();
    ...
    public void close() {
        // todo ? crazyness
        // todo [idea12] com.intellij.usages.UsageViewManager.getInstance(myProject).setCurrentSearchCancelled(true);
    ...
    public void addPerformOperationAction(@NotNull java.lang.Runnable processRunnable, java.lang.String commandName, java.lang.String cannotMakeString, @NotNull java.lang.String shortDescription, boolean checkReadOnlyStatus) {
    ...
    private Node[] getSelectedNodes() {
        ...
        final java.util.List<Node> result = new ArrayList<Node>();
    ...
    @NotNull
    public java.util.List<Usage> getSortedUsages() {
        java.util.List<Usage> usages = new ArrayList<Usage>(myUsages);
    ...
    private class MyPanel extends JPanel implements TypeSafeDataProvider, OccurenceNavigator {
        ...
        public void calcData(final DataKey key, final DataSink sink) {
            ...
            if (key == PlatformDataKeys.PROJECT) {
                ...
                sink.put(USAGE_VIEW_KEY, UsageView2Impl.this);
    ...
    private class JScrollPaneEe extends JScrollPane {
        JScrollPaneEe(JComponent view) {
            super(view);
        }
