...
public class STPPatternParser {
    ...
    static STPPattern parse(@NotNull String syntaxTreePattern) throws TokenStreamException {
        ...
        while((t = lexer.nextToken()).getType() != SyntaxTreePathLexer.EOF){
            if(t.getType() == SyntaxTreePathTypesTokenTypes.DOUBLE_SLASH){
                if(last != null){
                    last.addNode(iNode);
                    patterns.add(last);
                }
            ...
            } else if(t.getType() == SyntaxTreePathTypesTokenTypes.SLASH){
                if(last != null){
                    last.addNode(iNode);
                    patterns.add(last);
                }
                ...
                last.setHead("/");
                iNode = new InnerNodeImpl();
            } else if(t.getType() == SyntaxTreePathTypesTokenTypes.NUMBER){
                if(iNode.isComplete()){
                    last.addNode(iNode);
                    iNode = new InnerNodeImpl();
                }
                iNode.setPosition(Integer.parseInt(t.getText()));
            } else if(t.getType() == SyntaxTreePathTypesTokenTypes.DOLLAR){
                if(iNode.isComplete()){
                    last.addNode(iNode);
                    iNode = new InnerNodeImpl();
                }
                iNode.setAST(false);
            } else if(t.getType() == SyntaxTreePathTypesTokenTypes.SHARP){
                if(iNode.isComplete()){
                    last.addNode(iNode);
                    iNode = new InnerNodeImpl();
                }
                iNode.setAST(true);
            } else if(t.getType() == SyntaxTreePathTypesTokenTypes.IDENTIFIER){
                if(iNode.isComplete()){
                    last.addNode(iNode);
                    iNode = new InnerNodeImpl();
                }
                iNode.setText(t.getText());
            } else if(t.getType() == SyntaxTreePathTypesTokenTypes.DOUBLEDOT){
                if(iNode.isComplete()){
                    last.addNode(iNode);
                    iNode = new InnerNodeImpl();
                }
